		...
		switch(op){
			case 1:
			    ...
			    while(gameover==0){ // Enquanto o usuário não perder
			        ...
			        if(cx==comida[0] && cy==comida[1]){ // Se a cobra comer a comida, a velocidade da cobra aumenta
			            pontos++;
			            if(velo>50) velo-=10;
			            geraComida();
			        }
			        gameover=analizaSeBateu();
			        atualizaCobra(); 
			        desenhaCobra(); 
			        posicaoxy(50,20);
			        Sleep(velo);
			    }
			    system("cls"); // Limpa a tela e exibe uma mensagem final
